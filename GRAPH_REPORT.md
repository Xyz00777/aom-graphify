...
Cohesion: 0.05
Nodes (53): analyze_overhead(), OverheadStats, _quantile(), Linear-interpolation quantile (matches numpy's default).      Hand-rolled so we, Per-task overhead summary.      ``None`` fields signal "insufficient data": eith, Return the overhead summary for a recorded session's events.      Args:, _fmt_seconds(), format_diagnostics_section() (+45 more)
...
### Community 60 - "Per-Task Timing Tests"
Cohesion: 0.20
...
Cohesion: 0.07
Nodes (16): _AsyncEventWriter, Background thread that drains events onto disk.      R16: ``record_event`` enque, Enqueue a serialised JSONL line. Returns immediately.          Drops the line an, Cumulative count of events dropped because the queue was full., Reason string if the writer thread hit a disk error, else None.          Set onc, Wait until the queue is fully drained.          Used by tests that need to asser, Signal the writer thread to drain and exit., Consume the rest of the queue as no-ops after a disk error.          We can't wr (+8 more)
...
### Community 123 - "Host Collection Helpers"
Cohesion: 0.12
...
### Community 137 - "TestCoreModuleStructure"
Cohesion: 0.20
...
### Community 145 - "TestInspectSubcommand"
Cohesion: 0.20
...
Cohesion: 0.13
Nodes (3): FakeRenderer, Any, Minimal Renderer-shaped sink used by the driver smoke tests.
...
### Community 182 - "TestHideStateCompactPlumbing"
Cohesion: 0.17
...
### Community 210 - "Ansible Posix Availability"
Cohesion: 0.16
Nodes (13): _default_state_dir(), Any, Path, Shell-completion helpers for the AOM CLI (F5).  Two responsibilities:  1. ``sess, Resolve the default sessions directory.      Mirrors the literal used by ``inspe, Return session IDs under ``state_dir`` whose names start with ``prefix``.      T, session_id_completer(), _build_parser() (+5 more)
...
### Community 222 - "WarningType Enum"
Cohesion: 0.50
...
### Community 249 - "JSONL Event Parsing"
Cohesion: 0.18
...
### Community 256 - "KeyAction TypedDict"
Cohesion: 0.32
...
Cohesion: 0.21
Nodes (7): _pause_task_event(), Tests for ansible.builtin.pause-with-seconds visibility (IP5).  The pause module, ansible sometimes serialises args as strings; tolerate that., A pause-with-seconds task surfaces a sleeping log line., Some playbooks use the short ``pause`` action name., A prompt-style pause is handled by the runner stdin path, not here., TestPauseSecondsLogged
...
### Community 479 - "test_warning_color.py"
Cohesion: 0.44
